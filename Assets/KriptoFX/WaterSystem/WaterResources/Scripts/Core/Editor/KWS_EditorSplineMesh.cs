...
namespace KWS
{
    internal class KWS_EditorSplineMesh
    {
        ...
        readonly Color _handleStartColor = new Color(0.3f, 0.3f, 0.95f, 0.99f);
        ...
        Vector3 DrawMoveHandle(Vector3 position, Color color)
        {
            var newPos = position;
            Handles.color = color;
            var fmh_192_53_638550036099931257 = Quaternion.identity; newPos = Handles.FreeMoveHandle(newPos, GetHandleSphereSize(position), Vector3.one, Handles.CylinderHandleCap);
            return newPos;
        }
        ...
        private void DrawHandles(List<SplineScriptableData.SplinePoint> points, bool isSplineSelected)
        {
            ...
            for (var i = 0; i < points.Count; i++)
            {
                ...
                var freeMoveHandleNewPos = DrawMoveHandle(newPos, i == 0 ? _handleStartColor : handleColor);
        ...
        public void DrawSplineMeshEditor(Object target)
        {
            ...
            if (IsCanUpdateMesh() && _selectedSpline != null)
            {
                WaterInstance.SplineMeshComponent.UpdateSelectedMesh(WaterInstance, _selectedSpline);
